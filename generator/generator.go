...
func (g *Generator) Run() error {
	...
	// Discover and add referenced types from external packages iteratively
	// Keep discovering until no new types are found (handles transitive dependencies)
	for {
		initialCount := len(g.P.TypeNames)
		g.discoverReferencedAnnotatedTypes()

		// If no new types were discovered, we're done
		if len(g.P.TypeNames) == initialCount {
			break
		}
	}

	// After discovering external types with @GqlInclude, rebuild dependency graph
	// and apply auto-generation to newly discovered types
	if g.Config.AutoGenerate.Enabled && len(g.P.TypeNames) > 0 {
		depGraph := g.BuildDependencyGraph()
		depGraph.MarkTypesForGeneration(g.Config)
		g.applyAutoGeneration(depGraph)
	}

	// Re-run auto-generation after discovering all external types
	// This ensures that dependencies of external types (like PageInfo from Connection) are auto-generated
	if g.Config.AutoGenerate.Enabled {
		depGraph := g.BuildDependencyGraph()
		depGraph.MarkTypesForGeneration(g.Config)
		g.applyAutoGeneration(depGraph)
...
func (g *Generator) buildDependencyOrder() []string {
	// Include types from TypeNames (scanned types) and types marked for auto-generation
	nameSet := make(map[string]bool)

	// Add all types from TypeNames
	for _, name := range g.P.TypeNames {
		nameSet[name] = true
	}

	// Add all types marked for auto-generation (from external packages or dependencies)
	for typeName := range g.AutoGeneratedTypes {
		if _, exists := g.P.StructTypes[typeName]; exists {
			nameSet[typeName] = true
		}
	}

	for typeName := range g.AutoGeneratedInputs {
		if _, exists := g.P.StructTypes[typeName]; exists {
			nameSet[typeName] = true
		}
	}
...
func (g *Generator) generateByNamespace(orders []string) (map[string]string, error) {
	...
	for namespace, items := range namespaces {
		...
		// Context already created above		// Sort type names for deterministic output
...
func (g *Generator) generateByNamespaceAndPackage(orders []string) (map[string]string, error) {
	...
	for _, enumName := range g.P.EnumNames {
		...
		if ns != "" {
			...
			// Use package name
			pkgName := g.P.PackageNames[enumName]
	...
	for _, typeName := range orders {
		...
		// Process types
		if d.HasTypeDirective || g.AutoGeneratedTypes[typeName] {
			...
			for _, typeDef := range typeDefs {
				...
				if ns != "" {
					...
					// Use package name
					pkgName := g.P.PackageNames[typeName]
					outFile = filepath.Join(g.Config.Output, pkgName+g.Config.OutputFileExtension)
				}
		...
		// Process inputs
		if d.HasInputDirective || g.AutoGeneratedInputs[typeName] {
...
func (g *Generator) generateSingleFile(orders []string) (map[string]string, error) {
	...
	// Use a map of buffers to collect content (needed for concrete types)
	fileContents := make(map[string]*strings.Builder)
	buf := &strings.Builder{}
	fileContents[outFile] = buf
...
func (g *Generator) generatePackageFiles(orders []string) (map[string]string, error) {
	...
	for _, enumName := range g.P.EnumNames {
		...
		// Get package name from PackageNames
		pkgName := g.P.PackageNames[enumName]
...
func (g *Generator) generateTypeFromDef(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, typeDef TypeDefinition, ctx *GenerationContext) string {
	...
	fields := g.generateFieldsForTypeNamed(st, d, typeDef.IgnoreAll, false, name, typeName, ctx, "", nil)
...
func (g *Generator) generateInputFromDef(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, inputDef InputDefinition, ctx *GenerationContext) string {
	...
	fields := g.generateFieldsForTypeNamed(st, d, inputDef.IgnoreAll, true, inputName, typeName, ctx, "", nil)
...
func (g *Generator) generateFieldsForTypeNamed(st *ast.StructType, d StructDirectives, typeIgnoreAll bool, forInput bool, typeName string, goTypeName string, ctx *GenerationContext, fieldPrefix string, embeddedOpts *FieldOptions) string {
	buf := strings.Builder{}
	...
	for _, f := range st.Fields.List {
		...
		if f.Names == nil {
			...
			embeddedFields := g.expandEmbeddedFieldNamed(f, d, ignoreAll, forInput, typeName, ctx, fieldPrefix, embeddedOpts)
		...
		// Apply embedded field options if provided (BEFORE checking shouldIncludeField)
		if embeddedOpts != nil {
			// Inherit required/optional
			if embeddedOpts.Required && !opt.Required {
				opt.Required = true
			}
			if embeddedOpts.Optional && !opt.Optional {
				opt.Optional = true
			}
			// Inherit read/write constraints if field doesn't have its own
			if len(embeddedOpts.ReadOnly) > 0 && len(opt.ReadOnly) == 0 && len(opt.WriteOnly) == 0 && len(opt.ReadWrite) == 0 {
				opt.ReadOnly = embeddedOpts.ReadOnly
			}
			if len(embeddedOpts.WriteOnly) > 0 && len(opt.WriteOnly) == 0 && len(opt.ReadOnly) == 0 && len(opt.ReadWrite) == 0 {
				opt.WriteOnly = embeddedOpts.WriteOnly
			}
			if len(embeddedOpts.ReadWrite) > 0 && len(opt.ReadWrite) == 0 && len(opt.ReadOnly) == 0 && len(opt.WriteOnly) == 0 {
				opt.ReadWrite = embeddedOpts.ReadWrite
			}
			// Inherit include/ignore lists if field doesn't have its own
			if len(embeddedOpts.IgnoreList) > 0 && len(opt.IgnoreList) == 0 {
				opt.IgnoreList = embeddedOpts.IgnoreList
			}
			if len(embeddedOpts.IncludeList) > 0 && len(opt.IncludeList) == 0 {
				opt.IncludeList = embeddedOpts.IncludeList
			}
			// Inherit ignore flag
			if embeddedOpts.Ignore && !opt.Ignore {
				opt.Ignore = true
			}
		}
			...
			if ctx != nil && ctx.TypeSubstitutions != nil {
				if substituted, exists := ctx.TypeSubstitutions[ident.Name]; exists {
					...
					if g.isGenericInstantiationExpr(substituted) {
						...
						embeddedFields := g.expandEmbeddedFieldNamed(syntheticField, d, ignoreAll, forInput, typeName, ctx, fieldPrefix, embeddedOpts)
						buf.WriteString(embeddedFields)
						continue
					}
				}
		...
		// Apply field prefix if provided (from embedded struct tag)
		if fieldPrefix != "" {
			fieldName = fieldPrefix + strings.ToUpper(fieldName[:1]) + fieldName[1:]
		}
		...
		if opt.Type == "" {
			...
			if baseTypeName != "" && !g.isTypeInScope(baseTypeName) {
				// Try to load the type on-demand from the field expression to check for annotations
				// This handles types from packages not in the scan list
				if g.P.HasGQLAnnotations(f.Type, goTypeName) {
					// Type has GQL annotations - it's intentionally scoped, don't flag as out-of-scope
				} else if g.Config.AutoGenerate.SuppressGenericTypeWarnings && g.isTypeParameter(baseTypeName) {
					...
					g.OutOfScopeTypes[baseTypeName] = append(g.OutOfScopeTypes[baseTypeName], ref)
...
func (g *Generator) expandEmbeddedFieldNamed(f *ast.Field, d StructDirectives, ignoreAll bool, forInput bool, typeName string, ctx *GenerationContext, parentPrefix string, parentEmbeddedOpts *FieldOptions) string {
	// Parse the gql struct tag on the embedded field itself
	embeddedFieldOpt := ParseFieldOptions(f, g.Config)

	// If the embedded field is explicitly ignored, skip it entirely
	if embeddedFieldOpt.Ignore {
		return ""
	}

	// Get the prefix from the embedded field's name tag (e.g., gql:"prefix")
	// Combine with parent prefix if both exist
	fieldPrefix := parentPrefix
	if embeddedFieldOpt.Name != "" {
		if fieldPrefix != "" {
			fieldPrefix = fieldPrefix + strings.ToUpper(embeddedFieldOpt.Name[:1]) + embeddedFieldOpt.Name[1:]
		} else {
			fieldPrefix = embeddedFieldOpt.Name
		}
	}

	// Store embedded field options to apply to nested fields
	// Inherit all relevant flags from parent if not explicitly set
	embeddedOpts := embeddedFieldOpt
	if parentEmbeddedOpts != nil {
		// Inherit required/optional
		if parentEmbeddedOpts.Required && !embeddedOpts.Required {
			embeddedOpts.Required = true
		}
		if parentEmbeddedOpts.Optional && !embeddedOpts.Optional {
			embeddedOpts.Optional = true
		}
		// Inherit read/write constraints
		if len(parentEmbeddedOpts.ReadOnly) > 0 && len(embeddedOpts.ReadOnly) == 0 {
			embeddedOpts.ReadOnly = parentEmbeddedOpts.ReadOnly
		}
		if len(parentEmbeddedOpts.WriteOnly) > 0 && len(embeddedOpts.WriteOnly) == 0 {
			embeddedOpts.WriteOnly = parentEmbeddedOpts.WriteOnly
		}
		if len(parentEmbeddedOpts.ReadWrite) > 0 && len(embeddedOpts.ReadWrite) == 0 {
			embeddedOpts.ReadWrite = parentEmbeddedOpts.ReadWrite
		}
		// Inherit include/ignore lists
		if len(parentEmbeddedOpts.IgnoreList) > 0 && len(embeddedOpts.IgnoreList) == 0 {
			embeddedOpts.IgnoreList = parentEmbeddedOpts.IgnoreList
		}
		if len(parentEmbeddedOpts.IncludeList) > 0 && len(embeddedOpts.IncludeList) == 0 {
			embeddedOpts.IncludeList = parentEmbeddedOpts.IncludeList
		}
		// Inherit legacy flags
		if parentEmbeddedOpts.Ignore && !embeddedOpts.Ignore {
			embeddedOpts.Ignore = true
		}
	}
...
func (g *Generator) generateConcreteTypesFromInstantiations() error {
	...
	for concreteTypeName, inst := range g.GenericInstantiations {
		...
		if namespace != "" {
			...
			// No namespace - use default output file name
			if strings.HasSuffix(g.Config.Output, ".graphqls") || strings.HasSuffix(g.Config.Output, ".graphql") || strings.HasSuffix(g.Config.Output, ".gql") {
				// Old style: Output is the full file path
...
func (g *Generator) generateConcreteTypeContent(typeName string, typeSpec *ast.TypeSpec, st *ast.StructType, ctx *GenerationContext) string {
	...
	fields := g.generateFieldsForTypeNamed(st, d, false, false, typeName, goTypeName, ctx, "", nil)
	buf.WriteString(fields)
...
func (g *Generator) isTypeInScope(typeName string) bool {
	...
	// Check if it's auto-generated as a type
	if g.AutoGeneratedTypes[typeName] {
		return true
	}

	// Check if it's auto-generated as an input
	// For auto-generated inputs, the type name might be the base type + "Input" suffix
	if g.AutoGeneratedInputs[typeName] {
		return true
	}

	// Check if the type name is an auto-generated input name (BaseType + "Input")
	// Try stripping "Input" suffix and check if the base type is auto-generated
	if strings.HasSuffix(typeName, "Input") {
		baseTypeName := strings.TrimSuffix(typeName, "Input")
		if g.AutoGeneratedInputs[baseTypeName] {
			return true
		}
	}
	...
	// This is the primary registry that tracks all scanned types with their annotations
	if scannedInfo, exists := g.P.ScannedTypes[typeName]; exists {
		// If it has type, input, or include directives, it's definitely in scope
		if scannedInfo.HasTypeDirective || scannedInfo.HasInputDirective || scannedInfo.HasIncludeDirective {
			return true
		}
	}

	// For input names (ending with "Input"), check if the base type has @GqlInclude
	// @GqlInclude types generate both type and input variants
	if strings.HasSuffix(typeName, "Input") {
		baseTypeName := strings.TrimSuffix(typeName, "Input")
		if scannedInfo, exists := g.P.ScannedTypes[baseTypeName]; exists {
			if scannedInfo.HasIncludeDirective {
				return true
			}
		}
	}

	// Also check if typeName matches any generated type/input names from scanned types
...
// discoverReferencedAnnotatedTypes scans all struct fields and discovers types with GQL annotations
// from packages not in the scan list, adding them to the parser for generation
func (g *Generator) discoverReferencedAnnotatedTypes() {
	// Iterate through all scanned types
	for _, typeName := range g.P.TypeNames {
		typeSpec := g.P.StructTypes[typeName]
		...
		for _, field := range structType.Fields.List {
			// Try to discover annotated types from this field
			g.P.HasGQLAnnotations(field.Type, typeName)
		}
	}
}
