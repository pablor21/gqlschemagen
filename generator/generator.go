...
type Generator struct {
	...
	// Key: type name, Value: slice of field paths where it's referenced
	OutOfScopeTypes map[string][]string
...
func NewGenerator(p *Parser, config *Config) *Generator {
	...
	return &Generator{
		...
		OutOfScopeTypes:     make(map[string][]string),
		GeneratedItems:      []GQLSchemaItem{},
	}
}
...
func (g *Generator) Run() error {
	...
	case GenStrategySingle:
		err = g.generateSingleFile(orders)
	case GenStrategyPackage:
		err = g.generatePackageFiles(orders)
	default: // GenStrategyMultiple
		err = g.generateMultipleFiles(orders)
	}

	// Report out-of-scope types if any were found
...
// generateTypeContent generates GraphQL type definitions for a given struct
// This is a shared helper used by all generation strategies
func (g *Generator) generateTypeContent(typeName string, typeSpec *ast.TypeSpec, d StructDirectives) string {
	return g.generateTypeContentWithContext(typeName, typeSpec, d, nil)
}

// generateTypeContentWithContext generates GraphQL type definitions with tracking context
func (g *Generator) generateTypeContentWithContext(typeName string, typeSpec *ast.TypeSpec, d StructDirectives, ctx *GenerationContext) string {
	buf := strings.Builder{}
	...
	if d.HasTypeDirective {
		...
		for _, typeDef := range d.Types {
			typeContent := g.generateTypeFromDefWithContext(typeSpec, g.P.Structs[typeName], d, typeDef, ctx)
			if typeContent != "" {
		...
		typeContent := g.generateTypeFromDefWithContext(typeSpec, g.P.Structs[typeName], d, defaultTypeDef, ctx)
		if typeContent != "" {
			buf.WriteString(typeContent)
		}
	}
...
// generateInputContent generates GraphQL input definitions for a given struct
// This is a shared helper used by all generation strategies
func (g *Generator) generateInputContent(typeName string, typeSpec *ast.TypeSpec, d StructDirectives) string {
	return g.generateInputContentWithContext(typeName, typeSpec, d, nil)
}

// generateInputContentWithContext generates GraphQL input definitions with tracking context
func (g *Generator) generateInputContentWithContext(typeName string, typeSpec *ast.TypeSpec, d StructDirectives, ctx *GenerationContext) string {
	buf := strings.Builder{}
	...
	if d.HasInputDirective {
		...
		for _, inputDef := range d.Inputs {
			inputContent := g.generateInputFromDefWithContext(typeSpec, g.P.Structs[typeName], d, inputDef, ctx)
			if inputContent != "" {
		...
		inputContent := g.generateInputFromDefWithContext(typeSpec, g.P.Structs[typeName], d, defaultInputDef, ctx)
		if inputContent != "" {
			buf.WriteString(inputContent)
		}
	}
...
func (g *Generator) generateByNamespace(orders []string) error {
	...
	for namespace, items := range namespaces {
		...
		for _, enumName := range items.enums {
			...
			enumContent := g.generateEnumWithContext(enumType, ctx)
			if enumContent != "" {
		...
		for _, typeName := range typeNames {
			...
			for _, typeDef := range typeDefs {
				typeContent := g.generateTypeFromDefWithContext(typeSpec, structType, d, typeDef, ctx)
				if typeContent != "" {
					buf.WriteString(typeContent)
				}
			}
		...
		for typeName, inputDefs := range items.inputs {
			...
			for _, inputDef := range inputDefs {
				inputContent := g.generateInputFromDefWithContext(typeSpec, structType, d, inputDef, ctx)
				if inputContent != "" {
					buf.WriteString(inputContent)
				}
			}
...
func (g *Generator) generateSingleFile(orders []string) error {
	...
	for _, typeName := range orders {
		...
		buf.WriteString(g.generateTypeContentWithContext(typeName, typeSpec, d, ctx))
		buf.WriteString(g.generateInputContentWithContext(typeName, typeSpec, d, ctx))
	}
...
func (g *Generator) generateMultipleFiles(orders []string) error {
	...
	for _, enumName := range g.P.EnumNames {
		...
		enumContent := g.generateEnumWithContext(enumType, enumCtx)
		if enumContent != "" {
			buf.WriteString(enumContent)
		}
	}
...
func (g *Generator) generateTypeFromDef(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, typeDef TypeDefinition) string {
	return g.generateTypeFromDefWithContext(typeSpec, st, d, typeDef, nil)
}

// generateTypeFromDefWithContext generates a GraphQL type with generation context for tracking
func (g *Generator) generateTypeFromDefWithContext(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, typeDef TypeDefinition, ctx *GenerationContext) string {
	...
	// Register generated item if context is provided
	if ctx != nil {
...
func (g *Generator) generateInputFromDef(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, inputDef InputDefinition) string {
	return g.generateInputFromDefWithContext(typeSpec, st, d, inputDef, nil)
}

// generateInputFromDefWithContext generates a GraphQL input with generation context for tracking
func (g *Generator) generateInputFromDefWithContext(typeSpec *ast.TypeSpec, st *ast.StructType, d StructDirectives, inputDef InputDefinition, ctx *GenerationContext) string {
	typeName := typeSpec.Name.Name
	...
	for _, f := range st.Fields.List {
		...
		if opt.Type == "" {
			...
			if baseTypeName != "" && !g.isTypeInScope(baseTypeName) {
				fieldPath := fmt.Sprintf("%s.%s", typeName, fieldName)
				g.OutOfScopeTypes[baseTypeName] = append(g.OutOfScopeTypes[baseTypeName], fieldPath)

				// Handle based on configuration
				switch g.Config.AutoGenerate.OutOfScopeTypes {
				case OutOfScopeFail:
					// Error will be reported after all types are processed
					continue
...
func (g *Generator) reportOutOfScopeTypes() error {
	...
	for _, typeName := range typeNames {
		fields := g.OutOfScopeTypes[typeName]
		sort.Strings(fields)
	...
	msg.WriteString("     - warn: Log warnings but continue (current: warnings shown)\n")
...
func (g *Generator) countExcludedFields() int {
	count := 0
	for _, fields := range g.OutOfScopeTypes {
		count += len(fields)
	}
	return count
}
...
func (g *Generator) generateEnum(enumType *EnumType) string {
	return g.generateEnumWithContext(enumType, nil)
}

func (g *Generator) generateEnumWithContext(enumType *EnumType, ctx *GenerationContext) string {
	buf := strings.Builder{}
	...
	// Register the generated enum if context is provided
	if ctx != nil {
		g.registerGeneratedItem(GQLSchemaItem{
			OutputFile:    ctx.OutputFile,
